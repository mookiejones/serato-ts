...
function removeDriveRoot(
  absoluteSongPath: string,
  platformParam?: NodeJS.Platform | null,
) {
...
function selectExternalRoot(
  externalSongPath: string,
  platformParam: NodeJS.Platform | null = null,
) {
  const platform = platformParam || process.platform;
  if (platform === "win32") {
...
export const buildCrateFilepath = (
  filename: string,
  seratoFolder: string,
): string => {
